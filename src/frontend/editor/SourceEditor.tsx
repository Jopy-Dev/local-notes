 ...
 * Theme lives in cm-theme.ts; find decorations + sync in cm-find.ts. The
 * Markdown formatting toolbar (round 2, REQ-016) mounts above the editor
 * when requested. External draft replacements (reload/conflict resolution)
 * sync via dispatch, never remount.
 */
...
export function SourceEditor({ value, language, readOnly, onChange, ...props }: SourceEditorProps) {
    ...
    const view = new EditorView({
      ...
      state: EditorState.create({
        ...
        extensions: [
          ...
          basicSetup,
          // REQ-030: CodeMirror's contenteditable carries role="textbox" -
          // the accessible name must live on that element, not the host.
          EditorView.contentAttributes.of({ "aria-label": "Note source editor" }),
          cmFind(),
  ...
  const host = (
    <div
      ...
      className="min-h-0 min-w-0 flex-1 overflow-auto bg-surface-code [scrollbar-color:var(--color-border-strong)_transparent] [scrollbar-width:thin]"
    />
  );
  if (!props.toolbar || language !== "markdown") return host;
  return (
    <div className="flex min-h-0 min-w-0 flex-col">
      <MarkdownToolbar getView={() => viewRef.current} readOnly={readOnly} />
      {host}
    </div>
  );
}
